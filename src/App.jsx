...
const EMPTY_TASK = {
  title: '', priority: 'High', deadlineStr: '', subtasks: [],
  category: null, tags: [], isRecurring: false, recurrencePattern: null, recurrenceDays: [],
};
...
function App() {
  ...
  const [filters, setFilters] = useState({ search: '', tags: [], priorities: [], statuses: [] });
  ...
  const handleSubtaskTitleChange = (id, title) => {
    setNewTask(prev => ({ ...prev, subtasks: prev.subtasks.map(s => s.id === id ? { ...s, title } : s) }));
  };
  ...
  const handleCreateOrUpdateTask = async (e) => {
    ...
    try {
      ...
      const fields = {
        ...
        recurrencePattern: newTask.recurrencePattern || null,
        recurrenceDays: newTask.recurrenceDays || [],
      };
      if (editingTaskId) {
        await updateDoc(doc(db, 'tasks', editingTaskId), fields);
      } else {
  ...
  const handleSnooze = async (taskId, ms) => {
    const snoozedUntil = new Date(Date.now() + ms);
    await updateDoc(doc(db, 'tasks', taskId), { snoozedUntil, status: 'Snoozed' });
  };

  const handlePomodoroComplete = async () => {
    if (!pomodoroTask) return;
    await updateDoc(doc(db, 'tasks', pomodoroTask.id), {
      pomodoroCount: (pomodoroTask.pomodoroCount || 0) + 1,
    });
    setPomodoroTask(prev => prev ? { ...prev, pomodoroCount: (prev.pomodoroCount || 0) + 1 } : null);
  };

  const filteredTasks = useMemo(() => applyFilters(tasks, filters), [tasks, filters]);
  const pendingTasks = filteredTasks.filter(t => !DONE_STATUSES.includes(t.status) && t.status !== 'Snoozed');
  const completedTasks = filteredTasks.filter(t => DONE_STATUSES.includes(t.status));
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-white font-sans selection:bg-indigo-500/30 overflow-hidden flex flex-col relative">
      ...
      <main className="flex-1 overflow-y-auto z-10 custom-scrollbar pb-20">
                          ...
                          {completedTasks.map(task => (
                            <div key={task.id} className="p-4 rounded-2xl bg-white/5 border border-white/5 opacity-60 hover:opacity-100 flex items-center justify-between group relative transition-opacity">
                              ...
                              <div className="absolute right-4 flex gap-2">
                                <button onClick={() => handleRestoreTask(task.id)} className="p-1.5 text-indigo-300 opacity-50 hover:bg-indigo-500/20 rounded-lg hover:opacity-100 hover:scale-110 transition-all" title="Restore">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                                  </svg>
                                </button>
                                <button onClick={() => handleDeleteTask(task.id)} className="p-1.5 text-red-400 opacity-50 hover:bg-red-500/20 rounded-lg hover:opacity-100 hover:scale-110 transition-all" title="Delete">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {currentView === 'settings' && <SettingsPage tasks={tasks} />}
      </main>
